...
The data was originally collected by the Polytechnic Institute of Portalegre in Portugal to build machine learning models that predict a student’s outcome based on various socioeconomic factors and academic performance [1]. This was done to develop an analytics tool for the tutoring program to direct their efforts more effectively.[2]
...
We decided to use the simple logistic model to explain the phenomenon since it performs well and is an explainable model.
Due to the structure of the model, we cannot assess the percentage increase of each individual variable, but we can look at different odds ratios.

To evaluate the influence of gender,for example, we need to take the exp of the coefficient related to gender.
...
We built several models capable of predicting moderately well whether a student will graduate based on enrollment data. Such a tool could be used in various real-world applications. For example, universities could use it to identify students who are struggling to graduate on time and implement policies to address these issues.

Furthermore, we We also investigated whether the University of Portalegre awarded scholarships based on merit or need (i.e., coming from a low-income family). If scholarships were awarded based on merit, it would likely indicate that recipients graduate sooner simply because they are better students. However, we found [3] that scholarships are awarded based on need. This suggests that the program effectively supports students during their academic journey, and increasing funding for it would be beneficial.
...
[1] Valentim Realinho, Jorge Machado, Luís Baptista, & Mónica V. Martins. (2021). Predict students' dropout and academic success (1.0) [Data set]. Zenodo. 

https://doi.org/10.5281/zenodo.5777340

[2] Realinho, V.; Machado, J.;Baptista, L.; Martins, M.V. Predicting
Student Dropout and Academic Success. Data 2022, 7, 146.

https://doi.org/10.3390/data7110146

[3] Politècnico De Portalegrebolsas de estudo.

https://www.ipportalegre.pt/pt/estudantes/servicos-de-acao-social/bolsas-de-estudo/
