...
Given that the majority of the vast majority of students is single and only a few of them belong to one of the other categories we decided to create a binary variable indicating wether a student is single or not.
...
-the majority of single people manages to graduate in time, the same can't be said for the other two categories ;(table 6)

-people who enroll after completing secondary school tend to graduate in time more often than those who do not (note:portalegre university allows some students to attend courses without having having an high-school diploma);(table 7)
...
-students who left their parents home to study (i.e. the 'displaced' variable) have a higher graduation rate compared to those who did not;(table 14)

-students who receive a scholarship have a higher graduation rate than those who do not (76.0% vs 41.3%); (table 16)

-women seem much to graduate on time much more than men do (57.9% vs 35.2%). manca la tableeeeee

add info about age at enrollment

*da mettere summary per age at enrollment*
...
We looked for outliers in the age at enrollment variable. By plotting it's distribution it is possible to see that itdoes not follow a normal distribution. We removed all values which are outliers in the boxplot of values condictioned by the is_graduated distibution . We then perform a train-test split and remove the outliers from the training set only: this procedure improves the ecological validity of the model and reduces overfitting.
...
One of the first models we employ is logistic regression, this seems like a very robust choice given the fact that our research question revolves around finding a probability in a binary classification task.
...
summary(fit_logit_1)
```
We keep only significant variables in the model and then remove outliers from the dataset.
```{r, echo = T, message = F, results = 'hide', warning = F}
...
-being in a non-STEM course is associated with a decrease of 0.50218 in the log-odds of graduating, compared to being in a STEM course (reference group);

-having a mother with secondary education is associated with an increase of 0.95651 in the log-odds of graduating compared to having a mother with no education (reference group);

-having a mother with higher education is associated with an increase of 0.84373 in the log-odds of graduating compared to having a mother with no education (reference group);

-being male is associated with a decrease of 0.94168 in the log-odds of graduating compared to being female (reference group);

-being a scholarship holder is associated with a decrease of 2.18108 in the log-odds of graduating compared to not being a scholarship holder;

-each additional year of age at enrollment is associated with a decrease of 0.36608 in the log-odds of graduating.
...
# We want also see if there are some interesting interactions between variables
...
In logistic regression, interactions occur when the effect of one predictor variable on the outcome depends on the value of another independent variable.
*inserire se ci sono delle interazioni interessanti*
```{r, echo = F}
summary(fit_int_2)
...
it possible to see that some interactions modify the log odds of the "baseline" regression model, this suggests that there are complex relationships between our dataset's features
...
We then plot residuals for both models in order to gain even more insights about the data we are working with: by looking Q-Q residuals plot, it is evident that the residuals are not normally distributed, suggesting that the variables included in the model are insufficient to explain the data variability.
...
```{r, echo = F, message = F, out.height = '30%'}
library(MASS)
...
kable_styling(kable((round(t(fit_lda$scaling), 3)),
...
We intend to evalueate our models by plotting their respective ROC curves and computing some standard evaluation metrics, such as precision, accuracy, recall and f1 score.
...
The models we implemented perform in a fairly similar way, this fact tells us many important things about our data.
...
We were able to build several models that are able with a moderate degree of certanty whether a student will graduate or not just by looking at enrollment data.
