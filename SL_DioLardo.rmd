...
index_m_secondary <- which(data$Mother.s.qualification == 1 | # Secondary Education—12th Year of Schooling or Equivalent
                             data$Mother.s.qualification == 6 | # Frequency of Higher Education
                             data$Mother.s.qualification == 13 | # General commerce course
                             data$Mother.s.qualification == 15 | # Complementary High School Course
                             data$Mother.s.qualification == 22 | # General Course of Administration and Commerce
                             data$Mother.s.qualification == 23 |
                             data$Mother.s.qualification == 29 |
                             data$Mother.s.qualification == 31 |
                             data$Mother.s.qualification == 32)
index_m_higher <- which(data$Mother.s.qualification == 2 | # Higher Education—bachelor’s degree
                          data$Mother.s.qualification == 3 | # Higher Education—degree
                          data$Mother.s.qualification == 4 | # Higher Education—master’s degree
                          data$Mother.s.qualification == 5 | # Higher Education—doctorate
                          data$Mother.s.qualification == 30 | # Higher education—degree (1st cycle)
                          data$Mother.s.qualification == 33 | # Higher Education—master’s degree (2nd cycle)
                          data$Mother.s.qualification == 34) # Higher Education—doctorate (3rd cycle)
...
data1 <- data.frame(
  Var1 = sample(c("A", "B", "C"), 6, replace = T),
  Var2 = sample(c("D", "E", "F"), 6, replace = T),
  Var3 = sample(c("G", "H", "I"), 6, replace = T)
)

data2 <- data.frame(
  Var1 = sample(c("J", "K", "L"), 6, replace = T),
  Var2 = sample(c("M", "N", "O"), 6, replace = T),
  Var3 = sample(c("P", "Q", "R"), 6, replace = T)
)

data3 <- data.frame(
  Var1 = sample(c("S", "T", "U"), 6, replace = T),
  Var2 = sample(c("V", "W", "X"), 6, replace = T),
  Var3 = sample(c("Y", "Z", "A"), 6, replace = T)
)

# Creiamo e formattiamo le tabelle
table1 <- kable(data1, format = "latex", booktabs = T, col.names = c("Var1", "Var2", "Var3")) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

table2 <- kable(data2, format = "latex", booktabs = T, col.names = c("Var1", "Var2", "Var3")) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

table3 <- kable(data3, format = "latex", booktabs = T, col.names = c("Var1", "Var2", "Var3")) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
...
design_matrix_train <- model.matrix(Output ~ . - 1,
                                    data = train)

design_matrix_test <- model.matrix(Output ~ . - 1,
                                   data = test)
...
Come possiamo vedere dalla tabella precedente, a parte per la QDA i modelli risultano molto simili tra le metriche utilizzate, anche se QDA ha ottenuto la migliore Recall. 
Il che porta a pensare che non ci siano particolari differenze tra i modelli usati.
POSSIAMO PRENDERE I COEFFICIENTI DEI VARI MODELLI E CONFRONTARLI TRA DI LORO. 
ALTRI COMMENTI VARI.
...
Per come è strutturato il modello (inserendo l'intercetta) non si può valutare l'incremento percentuale di ogni singola variabile, ma si può osservare di quanto è il rapporto tra le variabili. Possiamo solo valurare solo gli odds ratio. 
...
Detto ciò bisogna ricordare che il modello riesce a prevedere correttamente solo il 67.7% dei casi, andando ad indicare che la parte restante è dovuta a fattori non considerati nel modello, come ad esempio la motivazione, la passione, la voglia di studiare, i docenti, ecc. 
Da un lato ci sintiamo soddisfatti di sapere che il modello riesce a dare una discreta rappresentazione del fenomeno, ma dall'altro lato siamo ancora più soddisfatti nel sapere che una buona fetta nella riuscita dello studente non dipenda da informazioni pregresse dell'inizio del percorso accademico. Bisogna ricordare che si sta valutando solo una situazione particolare nel Portogallo della durata di 10 di raccolta dati e che probabilmente l'università è privata.
