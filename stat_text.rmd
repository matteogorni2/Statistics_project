...
# Example data for other tables
prev_qual <- data.frame(
  `Previous.qualification` = c("Higher", "No Secondary", "Secondary"),
  Freq = c(204, 232, 3988)
)

nat_qual <- data.frame(
  Nationality = c("Portuguese", "Others"),
  Freq = c(4314, 110)
)

mother_qual <- data.frame(
  `Mother's qualification` = c("Higher", "No Secondary", "Secondary"),
  Freq = c(591, 234, 3599)
)

father_qual <- data.frame(
  `Father's qualification` = c("Higher", "No Secondary", "Secondary"),
  Freq = c(415, 3076, 933)
)

# Creating tables
prev_qual_table <- kable(prev_qual, caption = "Previous Qualification", format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

nat_qual_table <- kable(nat_qual, caption = "Nationality", format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

mother_qual_table <- kable(mother_qual, caption = "Mother's Qualification", format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

father_qual_table <- kable(father_qual, caption = "Father's Qualification", format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("striped", "hold_position"))

# Print tables side by side
cat(paste0(
  "\\begin{table}[H]\n",
  "\\begin{subtable}[t]{0.45\\textwidth}\n", prev_qual_table, "\\end{subtable}\n",
  "\\hfill\n",
  "\\begin{subtable}[t]{0.45\\textwidth}\n", nat_qual_table, "\\end{subtable}\n",
  "\\hfill\n",
  "\\begin{subtable}[t]{0.45\\textwidth}\n", mother_qual_table, "\\end{subtable}\n",
  "\\hfill\n",
  "\\begin{subtable}[t]{0.45\\textwidth}\n", father_qual_table, "\\end{subtable}\n",
  "\\caption{Additional Example Tables}\n",
  "\\end{table}\n"
))
...
**Logistic Regression**
One of the first model we decide to employ is logistic regression, this seems like an obvious choice given the fact that our research question revolves around finding a probability in a binary classification task.
...
```{r}
# Qui diocane(TOGLIERE QUESTO COMMENTO DOPO)
...
QUI METTERE QUALCOSA SU COME GESTIRE GLI OUTLIERS IN QUESTI MODELLI
